...
| `provider` | string | 否 | 提供商筛选：`outlook` / `imap` / `custom` / `gptmail` / `icloud_hme` / `cloudflare_temp_mail` |
...
- 当 `provider=icloud_hme` 且池中无可用别名时，服务会调用已配置的 HME 插件动态创建 `@icloud.com` 别名并直接返回领取结果
- 未指定 `provider` 时不会随机领取 HME 长期别名；HME 必须通过 `provider=icloud_hme` 显式领取
...
- `provider=icloud_hme` 是长期别名：显式传入 `project_key` 并成功完成后返回 `available`，同一 `caller_id + project_key` 后续不会再次领取该别名；失败、释放或租约过期不会写入成功占用记录，可重试
